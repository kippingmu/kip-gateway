...
COPY .mvn ./.mvn
COPY pom.xml ./
COPY app/gateway-web/pom.xml app/gateway-web/pom.xml
...
RUN --mount=type=cache,target=/root/.m2 \
    mvn -B -pl app/gateway-web -am dependency:go-offline

COPY app ./app

RUN --mount=type=cache,target=/root/.m2 \
    mvn -B -pl app/gateway-web -am -DskipTests clean package
